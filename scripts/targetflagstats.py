...
flagstatsfiles = glob.glob('{0:s}/output/diagnostic_plots/*-corr-flag-flagging-summary-*.json'.format(os.getcwd()))
flagstatsfiles.sort(key=lambda x: os.path.basename(x))

os.system('mkdir -p {0:s}/gtb_reports'.format(os.getcwd()))
g = open('{0:s}/gtb_reports/targetflagstats.txt'.format(os.getcwd()), 'w')
g.write('## Target flagged fraction\n\n')
